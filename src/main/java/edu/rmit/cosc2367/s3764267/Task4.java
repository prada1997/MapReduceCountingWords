...
public class Task4 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //setting combiner class for the job.
        job.setCombinerClass(IntSumReducer.class);

        //setting partitioner class for the job.
        job.setPartitionerClass(JobPartitioner.class);
        //setting reducer class for the job.
        job.setReducerClass(IntSumReducer.class);
        //setting the number of reducer for the job that is 2.
...
//                        LOG.debug("The Mapper Task of Pradhuman, s3764267: The word is " + word + " which is a " + extraLongWords + ".");
    ...
    //creating a partitioner for the job which will store the data in different output file.
    public static class JobPartitioner extends Partitioner<Text, IntWritable >
    {

        //storing the small and extra long words in output file 0 and medium and long words in output file 1.
        public int getPartition(Text key, IntWritable value, int noReduceTasks)
        {
            // Setting log-level to debugging.
            LOG.setLevel(Level.DEBUG);
            LOG.debug("The Partitioner Task of Pradhuman, s3764267");
            ...
            if(key.toString().equalsIgnoreCase("Small")||key.toString().equalsIgnoreCase("Extra Long"))
            {
                //storing the small and extra long words in output file 0.
                return 0;
            }
            else if(key.toString().equalsIgnoreCase("Medium")||key.toString().equalsIgnoreCase("Long"))
            {
                //storing medium and long words in output file 1.
                return 1 % noReduceTasks;
            }
            else
                return 0;
        }
